...
<table id="dataTable" class="table table-bordered table-hover text-nowrap">
  ...
  <tr>
    ...
    <th style="min-width:50px;" class="pointer" title="Click to sort"><i class="fa fa-sort"></i> Mac Address</th>
...
for ($i = 0; $i < $TotalReg; $i++) {
  ...
  $uprofile = $userdetails['profile'];
	$mac_address = $userdetails['mac-address']; 
  ...
  <td style='text-align:center;'>  <i class='fa fa-minus-square text-danger pointer' onclick="if(confirm('Are you sure to delete username (<?= $uname; ?>)?')){loadpage('./?remove-hotspot-user=<?= $uid; ?>&session=<?= $session; ?>')}else{}" title='Remove <?= $uname; ?>'></i>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
	  <i style="color:rgb(255, 193, 7);" class='fa fa-refresh pointer' onclick="if(confirm('Are you sure to refresh (<?= $uname; ?>)?')){loadpage('./?refresh-hotspot-user=<?= $uid; ?>&username=<?=$uname;?>&router_id=<?php echo $router_id; ?>&session=<?php echo $session;?>')}else{}" title='Refresh <?= $uname; ?>'></i>&nbsp&nbsp&nbsp
